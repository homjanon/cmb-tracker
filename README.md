...
仪表盘在「雪球大V 追踪」板块**上方**新增一张**小散持仓回撤提醒表**，用于按**近12个月最高盈利**动态跟踪个人持仓的盈利回落，按回落幅度分档提醒（2026-08-06 v3 定稿）。
...
- 近12个月最高价 `ytd_high`（前复权）：首跑/新增标的用**严格滚动12个月**（今天-365天）历史最高**播种**（A股/港股：腾讯 qfq；美股：yfinance 剔除分红；基金：东财 pingzhongdata 全量净值）；之后每日破新高则**上移**（同时刷新高点日期）；**高点实际发生日**距今>365天自动重播（滚动窗口）。
- **锚点粘滞修复（2026-09-03）**：此前播种把「播种当天」当高点日期（`ytd_high_date`），锚点虽旧、日期却是新的，导致「锚点滑出窗口→重播」永远不触发；叠加取数起点为 `year-01-01`（约20个月跨度）把窗口外旧高点捞进来——招行 2025-07-10 前复权高点 44.534 粘到 2026-09 仍作基准，盈利回落虚报 **9.47pp**（真实约 **1.9pp**）。修复后：①`ytd_high_date` 记录**高点实际发生日**；②播种窗口收紧为 `today-365`；③state 引入 `anchor_ver`（v1 旧 state 首次运行强制重播一次完成校准）。yfinance 口径同步改为「逐高点分红下修补正后取最高」，与"A股 qfq 锚定最新日"严格等价。
