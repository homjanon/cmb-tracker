...
- 年内最高价 `ytd_high` 以**自然年**为单位：首跑/跨年用 前复权日K（A股：腾讯 qfq；港股/美股：yfinance 剔除分红）或净值（基金）取年内最高**播种**；之后每日取实时价，若破前高则**刷新基准**（随新高滚动）。
...
- 港股/美股播种：yfinance 不复权日K + 手动剔除分红（纯前复权，与A股一致）；港股代码自动转雅虎4位格式（03968→3968.HK）
- 播种兜底：美股→Nasdaq 历史接口（不复权）；港股→腾讯 qfq 日K；基金→天天基金/东方财富净值（NAV 本身已除权，无需再调整）
