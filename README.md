...
| 最高盈利 | `(持仓以来最高价 − 成本) / 成本`（需填成本；未填显示 —） |

| 当前盈利 | `(当前价 − 成本) / 成本` |

| 盈利回撤 | 最高盈利 − 当前盈利（百分点，跌为绿） |

| 市场回撤 | `(持仓以来最高价 − 当前价) / 持仓以来最高价`（仅参考，不触发提醒） |

| 回撤提醒 | 当前盈利 ≥5% 且从最高盈利相对回撤 ≥10% 时显示红色「盈利回撤≥X%，考虑止盈」 |



**动态基准（核心）**

- 持仓以来最高价 `ytd_high`（前复权）：首跑/新增标的使用近3-5年历史最高**播种**（A股：腾讯 qfq 近3年，接口上限~800根；港股/美股：yfinance 近5年剔除分红；基金：东财净值近3年）；之后每日取实时价，破新高则**刷新基准**（只升不降）。

- **跨年不重置**（2026-08-06 用户决策）：基准连续跨年，避免 1 月开年即跌时「当年最高盈利」被刷新成开年价、止盈信号失效。

- 提醒阈值：当前盈利 ≥5% 且从最高盈利相对回撤 ≥10% → 红色「盈利回撤≥X%，考虑止盈」；盈利 <5% 的薄利润标的不提醒。
...
- 港股/美股播种：yfinance 近5年 不复权日K + 手动剔除分红（纯前复权，与A股一致）；港股代码自动转雅虎4位格式（03968→3968.HK）
- 播种兜底：美股→Nasdaq 历史接口（近3年，不复权）；港股→腾讯 qfq 日K（近3年）；基金→天天基金/东方财富净值近3年（NAV 本身已除权，无需再调整）

- 相关脚本：`scripts/query_stock.py`（价格/持仓以来最高）、`scripts/holdings_drawdown.py`（计算+状态）、`scripts/my_holdings_block.py`（注入 render_html.py 的 CSS/HTML/JS）



**本地预填持仓以来最高价（可选，借本地通达信/网络源一次性播种）**
...
> 本地依赖（akshare / pandas / baostock 等）需装在 Python 3.11+ 虚拟环境中；若用本机托管环境，请先 `cd scripts` 再运行 `python run_daily.py`（脚本内部按仓库根目录定位文件）。可选环境变量：`BIYING_API_KEY`（仅当东财 `REVENUE_RATIO` 缺失时，作为非息占比兜底抓取）。
...
│   ├── holdings_drawdown_state.json  # 持仓以来最高价动态基准状态（每日刷新、跨年不重置、提交）
