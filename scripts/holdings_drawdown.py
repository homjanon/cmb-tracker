...
- 价格：复用 query_stock（A/港美股/ETF 腾讯；场外基金 天天基金/东财）。

- 基准最高价 ytd_high（前复权口径，**滚动12个月窗口，跨年不重置**）：
  首跑/新增标的用近12个月历史最高播种；每日破新高则上移（同时刷新高点日期）；
  **锚点实际发生日滑出窗口（距今>365天）→ 重播**——2026-09-03 修复：
  此前播种把「播种当天」当高点日期，锚点虽旧、日期却是新的，重播永远不触发，
  旧高点粘滞（招行 2025-07-10 的 44.534 粘到 2026-09，回落虚报 9.47pp，
  真实仅约 1.9pp）。旧 state 兼容：无 anchor_ver 标记的条目首次运行强制重播一次。
...
# state 格式版本：≥2 表示 ytd_high_date 记录的是**高点实际发生日**
# （v1 播种时记的是播种当天，导致重播条件永不触发 → 检测到 v1 强制重播）
ANCHOR_VER = 2
...
def seed_ytd_high(code, qtype, year, today_str):
    """取近12个月最高价。返回 (high, high_date) 或 None。"""
    if qtype == 'fund':
...
def seed_state(holdings, year, today_str):
    ...
    窗口：近12个月（A股/港股 腾讯 qfq；美股 yfinance 剔除分红；基金 东财 pingzhongdata）。
    口径：均统一为前复权/纯剔除分红。播种后跨年不重置、滑出窗口自动重播。
    ytd_high_date 记录**高点实际发生日**（2026-09-03 修复，原记录播种当天）。
    GitHub Action 无通达信，故播种统一走网络源。
    """
    state = {'year': year, 'anchor_ver': ANCHOR_VER, 'items': {}}
    for h in holdings:
        got = seed_ytd_high(h['code'], h['type'], year, today_str)
        if got:
            yh, hd = got
            state['items'][h['code']] = {'ytd_high': yh, 'ytd_high_date': hd}
            print(f"[seed] {h['code']} {h['name']} 近12个月最高 {yh} @ {hd}")
        else:
...
def compute(holdings, state, today=None):
    ...
    legacy = state.get('anchor_ver', 1) < ANCHOR_VER  # 旧 state：日期=播种日，不可信
    state['anchor_ver'] = ANCHOR_VER
    derived = []
    for h in holdings:
        code = h['code']
        ...
        if not it or not it.get('ytd_high'):
            got = seed_ytd_high(code, h['type'], year, today_str)
            if got:
                it = {'ytd_high': got[0], 'ytd_high_date': got[1]}
            else:
                it = {'ytd_high': cur, 'ytd_high_date': today_str}
        elif legacy:
            # 旧 state 一次性校准：ytd_high_date 是播种日而非高点日，
            # 重播条件被污染，直接重取近12个月最高（2026-09-03 修复）
            got = seed_ytd_high(code, h['type'], year, today_str)
            if got:
                it = {'ytd_high': got[0], 'ytd_high_date': got[1]}
        elif (it.get('ytd_high_date')
              and (today - datetime.date.fromisoformat(it['ytd_high_date'])) > datetime.timedelta(days=365)):
            # 滚动12个月窗口：**高点实际发生日**滑出窗口 → 重播近12个月最高
            got = seed_ytd_high(code, h['type'], year, today_str)
            if got:
                it = {'ytd_high': got[0], 'ytd_high_date': got[1]}
        ...
        if profit_dd is not None:
            ...
            # 盈利时用 LEVEL_TEXT（按名义档=本次实际回落，2026-08-25 修复粘滞文案）
