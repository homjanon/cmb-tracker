...
评分采用分段线性映射，阈值参照谷子地对股份行/国有行的口径（详见 docs/scoring.md）。
...
def buy_signal(f: Fundamentals) -> dict:
    """
    谷子地式买入信号 + 动态买入区间。
    基于 PB（破净程度）与股息率，结合五维总分。
    ...
    if total >= 105 and sig in ("HOLD",):
        sig = "BUY"
        reason += "；五维总分极高，上调至买入"
    return {"signal": sig, "zone_low": None, "zone_high": None, "reason": reason}
