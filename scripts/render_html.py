...
from vip_profile_block import VIP_PROFILE_CSS, VIP_PROFILE_HTML, VIP_PROFILE_JS
...
(function(){
  ...
  tryUrl(API,true).catch(function(){return tryUrl(CDN,false);}).then(fill)
    ...
    html = html.replace("</style>", my_holdings_css() + XQ_TABLE_CSS
                                 + VIP_PROFILE_CSS + "</style>")
    ...
    html = html + xqm_inline_script + xq_script + myh_inline + "<script>" + XQ_TABLE_JS + "</script>" + "<script>" + my_holdings_js() + "</script>" + "<script>" + VIP_PROFILE_JS + "</script>" + "</body></html>"
